...
export async function GET(
  ...
  try {
    ...
    const response = await fetchFromTMDB(
      `https://api.themoviedb.org/3/search/person?query=${encodeURIComponent(decodedQuery)}&include_adult=false&language=en-US&page=1`
    );
                  ...
                  $each: [
                    {
                      ...
                      image: searchResults[0].profile_path,
    ...
    const err = error as { message?: string };
    console.error('Error in searchPerson API:', err.message);
    return NextResponse.json(
