...
export async function GET(
  ...
  try {
    ...
    const response = await fetchFromTMDB(
      `https://api.themoviedb.org/3/search/tv?query=${encodeURIComponent(decodedQuery)}&include_adult=false&language=en-US&page=1`
    );
    ...
    const err = error as { message?: string };
    console.error('Error in searchTv API:', err.message);
    return NextResponse.json(
