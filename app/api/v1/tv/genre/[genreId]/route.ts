...
export async function GET(
  ...
  try {
    ...
    const data = await fetchFromTMDB(
      `https://api.themoviedb.org/3/discover/tv?with_genres=${genreIdNum}&language=en-US&page=1&sort_by=popularity.desc`
    );

    return NextResponse.json({ success: true, content: data.results });
  } catch (error: unknown) {
    const err = error as { message?: string };
    console.log('Error in getTvsByGenre API:', err.message);
    return NextResponse.json(
